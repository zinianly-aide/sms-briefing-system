...
public interface TemplateMapper {
    ...
    @Insert("INSERT INTO briefing_template (name, category, content, status, owner, updated_at) VALUES (#{name}, #{category}, #{content}, #{status}, #{owner}, #{updatedAt})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Template template);

    @Update("UPDATE briefing_template SET name = #{name}, category = #{category}, content = #{content}, status = #{status}, owner = #{owner}, updated_at = #{updatedAt} WHERE id = #{id}")
